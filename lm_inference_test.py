...
class LmInferenceTest(unittest.TestCase):
  ...
  def setUpClass(cls):
    ...
    gin_search_paths = [
        os.path.join(_MELIAD_PATH.value.replace('\r', ''), 'transformer/configs'),
        os.getcwd(),
    ]

    vocab_path = os.path.join(r'./ag_ckpt_vocab/', 'geometry.757.model')

    lm.parse_gin_configuration(gin_file, gin_param, gin_paths=gin_search_paths)

    cls.loaded_lm = lm.LanguageModelInference(
        vocab_path, r'./ag_ckpt_vocab/', mode='beam_search'
    )
